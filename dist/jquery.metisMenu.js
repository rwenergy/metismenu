...
(function (global, factory) {
  ...
  var ClassName = {
    ...
    SECONDLEVEL: 'nav-second-level',
    THIRDLEVEL: 'nav-third-level'
  };
  ...
  function () {
    ...
    _proto.init = function init() {
      ...
      // add .mm-active to .active with sub-menu
      $(this.element).find(conf.parentTrigger + ".active").has(conf.subMenu).addClass(ClassName.ACTIVE).children(conf.subMenu).addClass(ClassName.SHOW);
      // click event for links without sub menu start
      // menu-level-3
      $("." + ClassName.METIS + " ." + ClassName.THIRDLEVEL).children(conf.parentTrigger).children(conf.triggerElement).on(Event.CLICK_DATA_API, function (e) {
        var items = $(self.element).find(conf.parentTrigger);
        var trgMenu = $(this).closest(conf.subMenu);
        var subMenu = items.children(conf.subMenu + "." + ClassName.SHOW + "." + ClassName.THIRDLEVEL).not(trgMenu);
        subMenu.config = conf;
        subMenu.siblings(conf.triggerElement).attr('aria-expanded', 'false');
        self.hide(subMenu);
      });
      // menu-level-2
      $("." + ClassName.METIS + " ." + ClassName.SECONDLEVEL).children(conf.parentTrigger).not(":has(" + conf.subMenu + ")").children(conf.triggerElement).on(Event.CLICK_DATA_API, function (e) {
        var items = $(self.element).find(conf.parentTrigger);
        var subMenu = items.children(conf.subMenu + "." + ClassName.SHOW + "." + ClassName.THIRDLEVEL);
        subMenu.config = conf;
        subMenu.siblings(conf.triggerElement).attr('aria-expanded', 'false');
        self.hide(subMenu);
      });
      // menu-level-1
      $(this.element).children(conf.parentTrigger).not(":has(" + conf.subMenu + ")").children(conf.triggerElement).on(Event.CLICK_DATA_API, function (e) {
        if (conf.toggle) {
          var paRent = $(self.element).find(conf.parentTrigger + "." + ClassName.ACTIVE).not(this);
          var subMenu = paRent.children(conf.subMenu);
          subMenu.siblings(conf.triggerElement).attr('aria-expanded', 'false');
          self.hide(subMenu);
        }
      });
    ...
    _proto.hide = function hide(element) {
      ...
      elem.parent(conf.parentTrigger).removeClass(ClassName.ACTIVE); // eslint-disable-next-line no-unused-expressions

      elem.height(elem.height())[0].offsetHeight;
      elem.addClass(ClassName.COLLAPSING).removeClass(ClassName.COLLAPSE).removeClass(ClassName.SHOW);

      // collapse all start
      //elem.children(this.config.parentTrigger).removeClass(ClassName.ACTIVE).children(this.config.subMenu).removeClass(ClassName.SHOW);
      // collapse all end
      ...
      var complete = function complete() {
        // check if disposed
        if (!conf || !_this2.element) {
          return;
        }

        if (_this2.transitioning && conf.onTransitionEnd) {
          conf.onTransitionEnd();
        }
    ...
    _proto.dispose = function dispose() {
      ...
      $(this.element).find(this.config.parentTrigger).off('click');
